...
```{r}
  ...
  html <- getURL(baseURL)
# The origin md use read_html(baseURL) but the network failed.
  html <- read_html(html)
```
...
```
At this point `textData` is a vector with 167 entries. The easiest way to inspect the contents of the vector is to convert it into a data frame and `View()` it.
...
```{r}
  ...
  rowStartIDs <- c(11,19,27,35,43,51,59)
  gamesPlayed <- data.frame(do.call(rbind,
                                    lapply(rowStartIDs,function(x) textData[x:(x+7)])))
  #do.call() function is commonly used to convert a list into a data frame.
...
```{r}
    ...
    # ignoring bye week
    rowStartIDs <- c(43,62,81,100,119,138,157)
...
*Based on old version of website, not working."
